...
**0.3.2 仍未生效（0.3.3 修正）**：DWM 的 35/36 调用返回 S_OK，但标题栏外观无变化——现场诊断发现窗口的 `DWMWA_USE_IMMERSIVE_DARK_MODE(20)` 为 0（从未开启深色标题栏），系统因此仍按浅色绘制标题栏并忽略自定义底色。**0.3.3 修复**：在 `ApplyCaptionTheme()` 中同时按主题设置属性 20（深色 = 1、浅色 = 0），再设置 35/36；已在用户当前窗口上手动验证属性 20 可由 0 置 1 并回读为 1。

**属性 20 也无效 → 最终改为自绘标题栏（0.3.3 定稿）**：即时实验中把属性 20 置 1 并回读确认，标题栏外观依旧无任何变化，说明**该环境下系统绘制的标题栏不接受应用的自定义配色**（三条路都试过：`AppWindow.TitleBar.BackgroundColor`、DWM 35/36、DWM 20）。最终方案：`ExtendsContentIntoTitleBar = true` + `SetTitleBar(AppTitleBar)`，在 XAML 顶部放一条 32dip 的 `Border`（跟随主题着色、左侧图标 + 标题文字），系统的最小化/最大化/关闭按钮叠画在其右侧，按钮颜色仍由 `AppWindow.TitleBar.Button*` 控制（该组属性一直生效）。用户实测三种状态（未悬停聚焦 / 悬停 / 未聚焦）配色全部正确。

**0.3.3 附带修正**：自绘标题栏最初误用 `AppWindow.TitleBar.Height` 覆盖 XAML 高度——该值是**物理像素**，150% 缩放下 48 被当作 48dip（=72 物理像素），导致标题栏高于系统按钮区域、图标与文字看似底部对齐。现固定为 32dip（与系统默认标题栏一致、随 DPI 自动换算），并保留 `RightInset` 右侧留白避免与按钮重叠。
