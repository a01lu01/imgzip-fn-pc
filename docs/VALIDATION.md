...
本次环境：macOS arm64；项目内 .NET SDK 10.0.400、PowerShell 7.6.5，不修改系统 PATH。核心与执行层控制台测试共 **31 组通过**，保留原有 15 组并新增 16 组生命周期回归；使用真实 PowerShell 入口和替代引擎，未访问 NAS，也未执行真实图像编码。0.3.3 的用户 Windows 实测记录保留在 `UI-ISSUES-0.3.0-0.3.3.md`，不代替本版本的 Windows 验收。

既有覆盖：配置导入不修改原文件、旧字段缺省值、主题持久化、预设/参数互斥、非法事件解析、未知状态恢复、预演不创建输出、不递归/递归、中文/空格/引号/美元符/与号路径、同格式参数省略、转换重名、已有目录编号、部分失败/异常引擎、成功文件配对统计、取消后子进程退出及持久事件、畸形/中断协议。NAS 替身覆盖实际 SSH 参数与脚本 Base64 传输、共享映射回 UNC、连通与引擎可用分离、断线/未确认取消保留 unknown、重新检查恢复。

新增回归覆盖：排队移除后零运行请求、取消与持久化名额分配竞争、PC/NAS unknown 持续占用名额及确认终态释放、重启后等待手动继续/移除、旧记录与 marker 迁移去重、损坏记录保留并提示、迁移写入失败仍注册待检查任务、queued/launching 写入失败零运行请求、草稿编辑与请求快照隔离、汇总关闭判断、取消全部任务不启动排队项、未知任务无法确认时不宣称停止、失败详情与 PC/NAS 输出按钮通知、延迟 UI 调度后的幂等收尾、终态清理失败后的恢复。

NAS 调度测试使用恢复后的 NAS 请求，在 Mac 上绕过 Windows 来源选择差异，断言确实收到两次 NAS 运行请求、同时只启动一项，并验证 PC 可同时运行。控制台测试使用串行同步上下文模拟 UI 调度，不打开或操作窗口。
...
- [ ] 独立启动为空草稿；`--path` 自动载入；运行中右键/拖入新来源可编辑草稿而不改变已提交任务；不同目录多选被拒绝。
- [ ] PC 并发设为 1：第一项运行、第二项排队，移除第二项后第一项结束也不启动第二项；快速取消与名额分配竞争不漏启动检查。
- [ ] PC/NAS 任务 unknown 后仍占名额；重新检查确认终态后，排队任务才按对应限制启动。
- [ ] 带排队任务仅关闭后重开：queued 显示“等待手动继续”，不会自动压缩；“继续”和“移除”均有效。launching/running 显示待检查，阻止超额启动。
- [ ] 从 0.3.3 升级后，旧 tasks 与 active-job 按 ID 合并，旧记录均待检查；损坏记录提示并保留，不漏注册可检查任务。
- [ ] 多项运行和排队时“取消并等待”处理全部任务；未知任务检查仍未确认时保留窗口，不显示已停止。
...
- [ ] 保留 0.3.3 的 A 版布局顺序、自绘标题栏、860 × 740 默认逻辑尺寸、位置记忆、固定任务区与内容滚动。
- [ ] 指定浅/深颜色，跟随系统及手动主题即时保存，重启还原。
- [ ] 100% / 150% / 200% 缩放；窗口尺寸调整与跨屏 DPI；长路径/长错误内容不遮挡主要操作。
- [ ] Tab 顺序、焦点可见、键盘选择预设/引擎/参数、Enter/Escape 对话框操作。
- [ ] 添加/拖入/清空、设置/导入/连接测试、空状态/执行中/成功/部分失败/不可用/取消中/未知状态均可理解；运行时草稿仍可操作。
- [ ] 待继续任务的“继续/移除”、失败详情、PC/NAS 输出按钮及错误文字及时刷新；成功任务自动移出；关闭提示覆盖全部任务。
